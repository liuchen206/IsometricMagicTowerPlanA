...
export default class VMEvent extends VMBase {
    ...
    onValueInit() {
        ...
        //             let _var = this.VM.getValue(this.watchPath);
        //             v.emit([_var, _var, this.watchPath.split('.')]);
        //         })
        //     }
        // } else {
        //     let max = this.watchPathArr.length;
        //     for (let i = 0; i < max; i++) {
        //         let _val = this.VM.getValue(this.watchPathArr[i], '?');
        //         if (Array.isArray(this.changeEvents)) {
        //             this.changeEvents.forEach(v => {
        //                 v.emit([_val, _val, this.watchPath.split('.')]);
        //             })
        //         }
        //     }
        // }
